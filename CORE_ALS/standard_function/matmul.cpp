...
// [[Rcpp::export]]
arma::mat row_spar(const mat& A, const mat& B, const uvec& sampleindex) {
  size_t i, j, k;
  size_t A_rows,  B_cols,sample_num;
  A_rows = A.n_rows;
  B_cols = B.n_cols;
  sample_num= sampleindex.n_elem;
  mat C(A_rows, B_cols, fill::zeros);
  ...
  #pragma omp parallel for
      for (i = 0; i < A_rows; ++i) {
        for  (j = 0; j < sample_num; ++j){
          for (k = 0; k < B_cols; ++k) {
            C(i, k) += A(i, sampleindex(j)) * B(sampleindex(j), k);
          }
        }
      }
      return C;
