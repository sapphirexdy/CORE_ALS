...
num_cols <- 3600
iter_n <- 5  # 迭代次数
dim_factors <- c(40,50,60,70) # 隐向量维度
denisity <- 0.5  # 稀疏度
...
for (t in 1:length(lambdas)) {
  ...
  for (k in 1:length(dim_factors)) {
    ...
    # 生成随机稀疏矩阵
    U_true <- generate_distribution_matrix(dim_factors[k], num_rows, 1)
    V_true <- t(generate_distribution_matrix(dim_factors[k], num_cols, 1))
    R_true <- U_true %*% V_true
    
    num_nan <- round(num_rows * num_cols * (1 - denisity))  # NaN数量
...
df_init <- function(subsample_sizes = subsampling_rates,
  ...
  for (i in 1:nrow(data)) {
    ...
    data$MSE_error[i] <- (max(rmse_values) - min(rmse_values))*2  # 最大值与最小值的差作为误差条
  }
